...
coord_t
play_random_move_omp(struct playout_setup *setup,
		 struct board *b, enum stone color,
		 struct playout_policy *policy,
		 unsigned long * seed)
{
...
play_random_omp:
		 ...
		 * internal board state, obviously. */
		assert(!policy->setboard || policy->setboard_randomok);
		board_play_random_omp(b, color, &coord, (ppr_permit) policy->permit, policy, seed);
...
int
play_random_game_omp(struct playout_setup *setup,
                 struct board *b, enum stone starting_color,
		 struct playout_amafmap *amafmap,
		 struct board_ownermap *ownermap,
		 struct playout_policy *policy,
		 unsigned long * seed)
{
	...
	while (gamelen-- && passes < 2) {
		coord_t coord = play_random_move_omp(setup, b, color, policy, seed);
